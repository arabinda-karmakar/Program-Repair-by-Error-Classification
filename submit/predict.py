...
def findErrorClass( X, k ):
	...
	yPred = npzModel.predict_proba(X)
	# Let us predict a random subset of the 2k most popular labels no matter what the test poin
	# Make sure we are returning a numpy nd-array and not a numpy matrix or a scipy sparse matrix
	yPred = np.insert(yPred,32, np.zeros((np.size(X,0),)), axis=1)
	yPred = np.insert(yPred,35, np.zeros((np.size(X,0),)), axis=1)
	yPred = np.insert(yPred,37, np.zeros((np.size(X,0),)), axis=1)
	yPred = np.argsort(yPred,axis=1)
	yPred = np.flip(yPred,axis = 1)+1
	yPred = yPred.T[:][0:k].T
	return yPred